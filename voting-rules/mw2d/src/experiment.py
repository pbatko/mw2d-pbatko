...
from pathlib2 import Path  # https://docs.python.org/dev/library/pathlib.html

from ballot_2d2 import BallotCalc
from visualize import drawVisualization
from winner import calculateWinner


class OldExperiment:

    @staticmethod
    def fromFile(input_file_path,
                 output_dir_path,
                 ballot_calc):
        # type: (Path, Path, BallotCalc) -> OldExperiment
    ...
    @staticmethod
    def fromCommandList(commands,
                        output_dir_path,
                        ballot_calc):
        # type: (list[str], Path, BallotCalc) -> OldExperiment

        return OldExperiment(commands=OldExperiment.__parseInputCommands(commands),
                             ballot_calc=ballot_calc,
                             output_dir_path=output_dir_path)

    def __init__(self, commands, ballot_calc, output_dir_path=Path(".")):
        # used for 2D
        self.C = []
        self.V = []
    ...
    def run(self, rm_in_file=False, rm_out_files=False):
        # PBATKO TODO rm tmp files
        for command in self.COMMANDS:
            self.__executeCommand(command)

    def __createOutputFilePath(self, output_file_name):
        # type: (str) -> Path
    ...
    @staticmethod
    def __generateUniform(x1, y1, x2, y2, N, Party):
        # type: (float, float, float, float, int, str) -> list
        (x1, x2) = (min(x1, x2), max(x1, x2))
        (y1, y2) = (min(y1, y2), max(y1, y2))
        return [(random() * (x2 - x1) + x1, random() * (y2 - y1) + y1) for i in range(N)]

    @staticmethod
    def __generateGauss(x, y, sigma, N, Party):
        # type: (float, float, float, int, str) -> list
        return [(gauss(x, sigma), gauss(y, sigma)) for i in range(N)]

    @staticmethod
    def __generateCircle(x, y, r, N, Party):
        # type: (float, float, float, int, str) -> list
        count = 0
        L = []
        while (count < N):
    ...
    def __saveGeneratedData(self, output_file_name_prefix):
    ...
    def __computeWinners(self, rule_name, k, output_file_name_prefix):
        ...
        # TODO pass pathlib2.Path object, not strings
        calculateWinner(str(generated_data_file_path), str(winner_output_file_path), rule_name, k)
    ...
    # READ DATA IN
    @staticmethod
    def __parseInputCommands(lines):
        # type: (list[str]) -> list[list[str]]
        parsedCommands = []

        for line in lines:
            parsedCommands += OldExperiment.__parseSingleCommand(line)

        return parsedCommands

    @staticmethod
    def __parseSingleCommand(commandString):
        # splits on any number of blank characters
        raw_words = commandString.split()
