...
from utils.utils import PathValidator
...
class OldHistogram:
    ...
    def writeHistFileSaner(self, output_path):
        # type: (Path) -> None
        with file(str(output_path), "w") as output:

            output.write("{0} {1}\n".format(self.W, self.H))

            for y in range(self.H):
                for x in range(self.W):
                    output.write("{0} ".format(self.HISTOGRAM[y][x]))
                output.write("\n")
