...
def register():
    ApprovalBasedRules.add(PAV_Annealing)
    ApprovalBasedRules.add(PAV_ReverseGreedy)
    ApprovalBasedRules.add(PAV_Greedy)
    ApprovalBasedRules.add(PAV_Genetic)
...
class PAV_Annealing(ApprovalBasedRuleBase):
    ...
    @classmethod
    def getScoreMapForCommitteeSize(clazz, k):
...
class PAV_Genetic(ApprovalBasedRuleBase):
    ...
    def apply(clazz, V, number_of_candidates, k):
        ...
        for _ in xrange(number_of_generations):
