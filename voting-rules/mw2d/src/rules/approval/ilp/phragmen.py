...
import _cplex_helpers
from rules._base import ApprovalBasedRuleBase
from rules._base import ApprovalBasedRules


def register():
    ApprovalBasedRules.add(PhragmenMax_ILP)
    ApprovalBasedRules.add(PhragmenVar_ILP)
...
class PhragmenMax_ILP(ApprovalBasedRuleBase):
    ...
    @classmethod
    def apply(cls, V, number_of_candidates, k):
    ...
    def computeCommitteeAndLoads(cls,
                                 ...
                                 V,
                                 quiet=True,
                                 lp_output_file_name=None,
                                 verify_boolean_variable=None):
        # type: (int, int, list[list[int]], bool, str, callable) -> (list[int], list[float])
...
class Phragmen_ILP():
    ...
    def solve_ilp_var(cls,
                      ...
                      quiet,
                      lp_output_file_name):
        # type: (list[list[int]], int, int, bool, str) -> (list[float], list[int])
        ...
        if lp_output_file_name is not None:
            _cplex_helpers.write(problem, lp_output_file_name)
    ...
    def solve_ilp_max(cls,
                      ...
                      quiet,
                      lp_output_file_name,
                      verify_boolean_variable):
        # type: (list[list[int]], int, int, list[float], bool, str) -> (float, list[float], list[int], float)
        ...
        def get_e_ij_name(voter1, voter2):
            # type: (int, int) -> str
        ...
        if not quiet:
            _cplex_helpers.print_all_variables(problem)
        ...
        epsilon_value = problem.solution.get_values(get_e_name())
        voters_load_vector = cls.get_load_vector_from_solution(problem, x_vc_names_byVoter)
        committee = cls.get_committee_from_solution(problem, number_of_candidates=number_of_candidates, k=k)

        return epsilon_value, voters_load_vector, committee

    @classmethod
    def get_load_vector_from_solution(self, problem, x_vc_names_byVoter):
        load_vector = []
    ...
    @classmethod
    def get_committee_from_solution(cls, problem, number_of_candidates, k):
        # https://www.ibm.com/support/knowledgecenter/SSSA5P_12.8.0/ilog.odms.cplex.help/CPLEX/Parameters/topics/EpInt.html
    ...
    @classmethod
    def x_vc_name(cls, c, v):
        # type: (int, int) -> str
        # load received be voter v from candidate v
        return "x_v{0}c{1}".format(v, c)

    @classmethod
    def get_x_c_name(cls, c):
        # type: (int, int) -> str
        # total load induced by candidate c
        return "x_c{0}".format(c)

    @classmethod
    def get_x_c_name_inverse(cls, name):
        # type: (str) -> int
        return int(name[3:])

    @classmethod
    def set_fundamental_phragmen_constraints(cls, V, k, number_of_candidates, problem):
    ...
    @classmethod
    def draw_bar_graph(cls, values, output_file_name=None):
