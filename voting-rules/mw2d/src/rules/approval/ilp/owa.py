...
def register():
    ApprovalBasedRules.add(PAV_ILP)
    ApprovalBasedRules.add(CC_ILP)
...
class ApprovalOWA_ILP():
    ...
    @classmethod
    def apply(clazz, V, number_of_candidates, K, owa):
        problem = cplex.Cplex()
        ...
        def get_x_j_name_inverse(name):
            # type: (str) -> int
            return int(name[3:])

        def get_x_ijk_name(v, c, k):
        ...
        from _cplex_helpers import add_boolean_variable
        ...
        _cplex_helpers.write(problem, "_owa_ilp_pbatko.lp")

        problem.solve()

        committee = _cplex_helpers.get_committee_from_boolean_variable_names(
            problem=problem,
            number_of_candidates=number_of_candidates,
